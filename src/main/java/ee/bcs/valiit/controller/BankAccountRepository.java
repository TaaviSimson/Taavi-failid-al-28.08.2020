...
public class BankAccountRepository {
    ...
    public List<TransactionHistory> transactionHistory(Integer accountNr){
        String sql = "SELECT * FROM transaction_history " +
                "WHERE account_from_id = :account_from_id";   //Küsib ülekannete ajalugu kohal user id
        int id = getUserId(accountNr);
        Map<String, Object> paramMap = new HashMap();
        paramMap.put("account_from_id", id);
        return jdbcTemplate.query(sql, paramMap, new TransactionHistoryRowMapper());
    }
